...
const Game = {
  ...
  coinsCollected: 0,
  highScores: {
    easy: parseInt(localStorage.getItem("dangerDashHighScoreEasy")) || 0,
    normal: parseInt(localStorage.getItem("dangerDashHighScoreNormal")) || 0,
    hard: parseInt(localStorage.getItem("dangerDashHighScoreHard")) || 0,
  },
  gameSpeed: 2.2,
  difficulty: "normal",
  islandTimer: 0,
  ...
  createIsland() {
    ...
    // Use wider gaps and islands on mobile
    const isMobile = window.innerWidth <= 768;
    const gapMin = isMobile
      ? settings.mobileIslandGapMin
      : settings.islandGapMin;
    const gapMax = isMobile
      ? settings.mobileIslandGapMax
      : settings.islandGapMax;
    const gap = gapMin + Math.random() * (gapMax - gapMin);

    const minWidth = isMobile
      ? CONFIG.ISLAND.mobileMinWidth
      : CONFIG.ISLAND.minWidth;
    const maxWidth = isMobile
      ? CONFIG.ISLAND.mobileMaxWidth
      : CONFIG.ISLAND.maxWidth;
    const width = minWidth + Math.random() * (maxWidth - minWidth);

    const yVariation = -15 + Math.random() * 30;
    const newY = Math.max(
      CONFIG.ISLAND.minY,
      Math.min(CONFIG.ISLAND.maxY, lastIsland.y + yVariation)
    );
  ...
  createCoins() {
    ...
    for (let island of this.islands) {
      if (
        island.x > -50 &&
        island.x < CONFIG.CANVAS_WIDTH + 200 &&
        !island.hasCoins
      ) {
  ...
  gameOver() {
    ...
    if (this.coinsCollected > currentHighScore) {
      this.highScores[this.difficulty] = this.coinsCollected;
